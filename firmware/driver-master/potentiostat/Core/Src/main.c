...
/* Why the MCU last rebooted, captured from the sticky reset flags at boot and
 * reported to the host by the diagnostics query. Lets the software tell a
 * watchdog reset (firmware froze mid-run) from a normal power-up. */
uint32_t reset_cause_raw = 0;
...
int main(void)
 {
  ...
  /* Snapshot the reset-cause flags first, then clear them so the next boot
   * reports only its own cause. */
  reset_cause_raw = RCC->CSR;
...
/*  if (MX_FATFS_Init() != APP_OK) {
  ...
  DAC_Init(GetDAC_Cfg_Dflt());
  ADC_Init(GetAnalog_cfg_dflt());
  BlinkLed_Init();
  Humidity_Sensor_Task(&hi2c2);	// Function Initialization for SHTC3 humidity and temperature sensor
  Set_TIA_Gain(eTIA_SW_GAIN_100); // Selected resistor
  HAL_GPIO_WritePin(CE_EN_GPIO_Port, CE_EN_Pin, 0); // Set switch Off
  ...
  const osThreadAttr_t PstatTask_attributes = {
    .name = "Pstat_Task",
...
//  const osThreadAttr_t Triangle_attributes = {
//    .name = "Triangle_Task",
//    .priority = (osPriority_t) osPriorityNormal,
//    .stack_size = 512
//  };

  osThreadId_t pstat_thread = osThreadNew(Pstat_RunTime, NULL, &PstatTask_attributes);
  UNUSED(pstat_thread);

 // osThreadId_t LedBlink_thread = osThreadNew(BlinkLed_RunTime, NULL, &LedBlinkTask_attributes);
 // UNUSED(LedBlink_thread);

// osThreadId_t Triangle_thread = osThreadNew(Triangle_Test, NULL, &Triangle_attributes);
// UNUSED(Triangle_thread);
